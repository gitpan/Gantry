...
sub create {
    ...
    unless ( defined $value_for->{ $primary } ) {
...
in in the internals of that module, while still providing most of its
...
code.  The result is code which is easy to read, understand, and modify.

=head1 RATIONALE

Class::DBI and its cousins provide a beautiful API for client code.  By
...
Relation Mappers (ORMs) do a lot of work at run time.  Namely, they build
...
I could create, retrieve, update, and delete to my hearts content while
...
generated by magic during run time, my code is generated by simple grammar
...
its data models and controllers.  Then, when a programmer wonders what
...
pairs you want to match in a simple list.  Add a single hash reference as
the last parameter if you like.  Currently that hash reference may only
contain an order_by key.  Its value is used to fill in the blank
'ORDER BY ___'.
...
you want to populate.  Any value will be quoted if its value in the
get_quoted_cols hash is defined.
...
=item get_quoted_cols

Return a hash reference whose keys are the names of columns whose values should
be single quoted in SQL statements.  The values are unimportant, so long
as they are defined.
