package Gantry::Control::C::Authz;
...
sub handler : method {
	...
	if ( $user ) {
		...
		# get groups for user
		my @group_rows = Gantry::Control::Model::auth_group_members->search( 
		...
		for my $entry ( @$requires ) {
			...
			if ( $req eq 'valid-user' ) {
				...
				for ( @rest ) { 
					...
					return( $self->status_const( 'OK' ) ) if ( $user eq $_ );
				}
			}
			elsif ( $req eq 'group' ) {
				for ( @rest ) { 
...
Gantry::Control::C::Authz - Database based authorization.
